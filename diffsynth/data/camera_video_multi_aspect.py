...
class CameraVideoDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, npz_path, extend_paths, steps_per_epoch, max_num_frames=100, frame_interval=1, num_frames=81, height=480, width=832, is_i2v=False, is_camera=False):
        ...
        for entry in metadata:
            # if 'MiraData9K' not in entry['video_path'] and os.path.exists(os.path.join(base_path, entry['video_path'])):
    ...
    def load_simple_video(self, video_path):
    ...
    def __getitem__(self, index):
        ...
        if self.is_camera:
            ...
            if self.camera_extrinsics[data_id].shape[0] >= max(cam_idx):
                render_mask_frames = self.load_simple_video(self.render_mask_path[data_id])
                render_mask = torch.stack([ToTensor()(frame) for frame in render_mask_frames], dim=0)[:, 0:1]  # [f,1,h,w]
                render_mask = F.interpolate(render_mask, size=(self.height, self.width), mode='nearest')  # [0,1],[1,f,1,h,w]
                render_mask = einops.rearrange(render_mask, "f c h w -> c f h w")
                render_mask[render_mask < 0.5] = 0
                render_mask[render_mask >= 0.5] = 1
                data['render_mask'] = render_mask
                data["camera_extrinsics"], data["camera_intrinsics"] = get_camera_sparse_embedding(self.camera_extrinsics[data_id][cam_idx], self.camera_intrinsics[data_id][cam_idx], self.height, self.width)
                ...
                render_mask_frames = self.load_simple_video(self.render_mask_path[data_id])
                render_mask = torch.stack([ToTensor()(frame) for frame in render_mask_frames], dim=0)[:, 0:1]  # [f,1,h,w]
                render_mask = F.interpolate(render_mask, size=(self.height, self.width), mode='nearest')[None]  # [0,1],[1,f,1,h,w]
                render_mask = einops.rearrange(render_mask, "b f c h w -> b c f h w")
                render_mask[render_mask < 0.5] = 0
                render_mask[render_mask >= 0.5] = 1
                if self.is_i2v:
                    video_id, video, first_frame, max_num_frames = video
                    data = {"text": text, "video": video, "path": path, "first_frame": first_frame, "video_id": video_id, "max_num_frames": max_num_frames, "render_mask": render_mask}
                else:
                    video_id, video, max_num_frames = video
                    data = {"text": text, "video": video, "path": path, "video_id": video_id, "max_num_frames": max_num_frames, "render_mask": render_mask}
