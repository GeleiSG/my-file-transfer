...
def save_video(frames, save_path, fps, quality=9, ffmpeg_params=None, batch_size=1000):
    writer = imageio.get_writer(save_path, fps=fps, quality=quality, ffmpeg_params=ffmpeg_params)
    
    total_frames = len(frames)
    for i in tqdm(range(0, total_frames, batch_size), desc="Saving video"):
        batch = frames[i:i+batch_size]
        for frame in batch:
            frame = np.array(frame)
            writer.append_data(frame)
            del frame  # 删除已处理的帧，释放内存
        del batch  # 删除当前批次，释放内存
