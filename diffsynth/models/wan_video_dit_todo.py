...
class WanModel(torch.nn.Module):
    ...
    def build_controlnet(self):
        ...
        self.controlnet_patch_embedding = nn.Conv3d(
            self.in_dim, self.controlnet_cfg.conv_out_dim, kernel_size=self.patch_size, stride=self.patch_size
        )
        ...
        # self.time_projection = nn.Sequential(nn.SiLU(), nn.Linear(self.controlnet_cfg.dim, self.controlnet_cfg.dim * 6))
    ...
    def forward(self,
        ...
        if render_latent is None or (enable_render_drop and random.random() < 0.1):
            add_inputs = self.controlnet_mask_embedding(camera_embedding)
            controlnet_rotary_emb = self.controlnet_rope(x)
        ...
        x, (f, h, w) = self.patchify(x, camera_embedding)
                ...
                if self.training and use_gradient_checkpointing:
                    if use_gradient_checkpointing_offload:
                        with torch.autograd.graph.save_on_cpu():
                            ...
                            #     use_reentrant=False,
                            # )
                            x = create_custom_forward(block)(x, context, t_mod, freqs)
                    else:
                        # x = torch.utils.checkpoint.checkpoint(
                        #     create_custom_forward(block),
                        #     x, context, t_mod, freqs,
