...
class WanModel(torch.nn.Module):
    def __init__(
        ...
        # self.camera_adapter = CameraPoseControlNet( 
        #     downscale_factor=8,
        #     channels=[64, 128, 320],
        #     cin=384,
        #     cout=dim,
    ...
    def patchify(self, x: torch.Tensor, camera_embedding: torch.Tensor = None):
        ...
        if camera_embedding is not None and self.cam_adapter is not None:
    ...
    def forward(self,
                ...
                enable_render_drop = True,
                enable_camera_drop = True,
                **kwargs,
                ):
        ...
        if camera_embedding is None or (enable_camera_drop and random.random() < 0.05):
            ...
            if render_mask is not None:
                ...
                one_render_mask = torch.ones_like(camera_embedding[:,0:1,:,:,:]).to(camera_embedding.device, camera_embedding.dtype)
                add_inputs = torch.cat([one_render_mask, camera_embedding], dim=1)
            add_inputs = self.controlnet_mask_embedding(add_inputs)

        # downsampled_mask = F.interpolate(
        #     render_mask, 
        #     size=(f, h, w), # 1. 指定目标的时间、高度、宽度
        #     mode='trilinear',              # 2. 使用三线性插值，对时空维度都进行平滑处理
        #     align_corners=False          # 建议设置为False，是新版的默认行为
        # )
