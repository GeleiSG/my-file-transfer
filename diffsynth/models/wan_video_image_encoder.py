...
class WanImageEncoder(torch.nn.Module):
    ...
    def encode_image(self, videos):
        # preprocess
        assert videos.ndim == 4, f"输入必须是一个4D的批次张量 (B, C, H, W)，但实际得到的是 {videos.ndim}D"
        size = (self.model.image_size,) * 2
        # test_videos_list = [videos]
        # origin_videos = torch.cat([
        #     F.interpolate(
        #         u,
        #         size=size,
        #         mode='bicubic',
        #         align_corners=False) for u in test_videos_list
        # ])
