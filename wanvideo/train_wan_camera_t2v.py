...
class LightningModelForDataProcess(pl.LightningModule):
    ...
    def test_step(self, batch):
        ...
        # camera_emb = batch["camera_embedding"][0]
        # if not os.path.exists(path + ".abs_tensors.pth"):
        if True:
            ...
            if video is not None:
                ...
                if "first_frame" in batch:
                    ...
                    image_emb = {}
                video_id = torch.stack(video_id, dim=1)[0].cpu().numpy()
                data = {"latents": latents, "prompt_emb": prompt_emb, "image_emb": image_emb, "video_id": video_id}
                # data = {"latents": latents, "prompt_emb": prompt_emb, "image_emb": image_emb, "video_id": video_id, "max_num_frames": batch["max_num_frames"].item()}
                # new_path = os.path.join('data/temp_data', path.split('/')[-1])
                if os.path.exists(f"{path}.tensors.pth"):
                    os.system(f"rm {path}.tensors.pth")
                torch.save(data, path + ".tensors.pth")
        # else:
        #     current_data = torch.load(path + ".valid_tensors.pth", weights_only=True, map_location="cpu")
        #     current_data["align_factor"] = batch["align_factor"]
        #     torch.save(current_data, path + ".valid_tensors.pth")
...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, npz_path, steps_per_epoch):
        ...
        self.path = [os.path.join(base_path, entry["video_path"]) for entry in metadata]
...
class LightningModelForTrain(pl.LightningModule):
    def __init__(
        ...
        for block_i, block in enumerate(self.pipe.dit.blocks):
                ...
                # block.cam_encoder.residual_blocks[-1].conv2.weight.data.zero_()
                # block.cam_encoder.residual_blocks[-1].conv2.bias.data.zero_()
            # if block_i < 20:
        ...
        if resume_ckpt_path is not None:
            if train_architecture == "adapter":
                ...
                try:
                    self.pipe.dit.load_state_dict({k: state_dict[k] for k in full_model_keys}, strict=False)
        ...
        if train_architecture == "adapter":
            for block_i, block in enumerate(self.pipe.denoising_model().blocks):
                # if block_i < 20:
            ...
            for name, module in self.pipe.denoising_model().named_modules():
                if any(keyword in name for keyword in ["cam_encoder", "projector"]):
                    print(f"Trainable: {name}")
                    for param in module.parameters():
                        param.requires_grad = True
    ...
    def training_step(self, batch, batch_idx):
        ...
        # camera_embedding = batch["camera_embedding"]
        ...
        #     camera_plucker_emb = batch["camera_plucker_embedding"][0].to(self.device)
        #     # Encode camera
            # camera_embedding = batch["camera_embedding"].to(self.device)
    ...
    #     print(camera_extrinsic[video_id].cpu().numpy()[:,:3,:].shape)
    #     ccc
    #     # generate video
    ...
    #     vggt_model.eval()
    #     with torch.no_grad():
    #         estimated_extrinsic, estimated_intrinsic = generate_camera_params(vggt_model, video_path, 'data/train_eval/temp_images') # (81, 3, 4) (81, 3, 3)
    #     del vggt_model
    #     torch.cuda.empty_cache()
    #     # calculate metric
    #     gt_extrinsic = camera_extrinsic[video_id].cpu().numpy()[:,:3,:]
    #     print(gt_extrinsic.shape)
    #     print(estimated_extrinsic.shape)
    #     trans_err, rot_err, cam_mc = calculate_pose_errors(gt_extrinsic, estimated_extrinsic)
    #     self.log("trans_err", val_loss, on_epoch=True, prog_bar=True)
    #     self.log("rot_err", val_loss, on_epoch=True, prog_bar=True)
    #     self.log("cam_mc", val_loss, on_epoch=True, prog_bar=True)
...
def data_process(args):
    dataset = CameraVideoDataset(
        args.dataset_path,
        os.path.join(args.dataset_path, 'RealCam-Vid_train_camera.npz'),
        # os.path.join(args.dataset_path, 'MultiCamVideo_camera.npz'),
        ...
        is_camera=False   # 确保启用 camera 相关字段
    )
...
def train(args):
    ...
    #     args.dataset_path,
    #     'data/RealCam-Vid_DL3DV_test.npz',
    #     steps_per_epoch=args.steps_per_epoch,
    # )
    dataset = CameraVideoDataset(
        args.dataset_path,
        os.path.join(args.dataset_path, 'RealCam-Vid_train_camera.npz'),
        ['/root/hdd/user_workspace/yuqifan/cameractrl/data/test_vggt.json'],
        steps_per_epoch=args.steps_per_epoch,
        max_num_frames=129,
        frame_interval=1,
        num_frames=81,
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[pl.pytorch.callbacks.ModelCheckpoint(save_top_k=-1, every_n_train_steps=500)],
        logger=logger,
    )
