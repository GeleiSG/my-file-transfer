...
class LightningModelForTrain(pl.LightningModule):
            ...
            for name, param in self.pipe.denoising_model().named_parameters():
                if param.requires_grad:
                    ...
                    if 'lora_' in name:
                        ...
                        pass  # 常规参数冻结，无需打印
        elif train_architecture == "controlnet":
            for name, module in self.pipe.denoising_model().named_modules():
                if any(keyword in name for keyword in ["controlnet"]):
                    print(f"Trainable: {name}")
                    for param in module.parameters():
                        param.requires_grad = True
            trainable_params = 0
    ...
    def training_step(self, batch, batch_idx):
        ...
        if video is not None:
            ...
            if torch.all(batch["camera_extrinsics"]==0):
                ...
                camera_embedding = get_plucker_embedding(batch["camera_extrinsics"], batch["camera_intrinsics"], height, width, device=self.device).to(device=self.device)
        ...
        # )
        noise_pred = self.pipe.denoising_model()(
            noisy_latents, timestep=timestep, render_latent=render_latent, render_mask=render_mask, camera_embedding=camera_embedding, **prompt_emb, **extra_input, **image_emb,
            use_gradient_checkpointing=self.use_gradient_checkpointing,
            use_gradient_checkpointing_offload=self.use_gradient_checkpointing_offload
        )
    ...
    def on_save_checkpoint(self, checkpoint):
        if self.train_architecture == 'adapter':
            ...
            for name, param in state_dict.items():
                if name in trainable_param_names:
                    train_state_dict[name] = param
            torch.save(train_state_dict, os.path.join(checkpoint_dir, f"step{current_step}.ckpt")) # save all params
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default="lora",
        choices=["lora", "full", "adapter", "controlnet"],
...
def collate_fn(examples):
    ...
    for key in examples[0]:
        new_examples[key] = []
    batch_video_length = 81

    for example in examples:
        for key in example:
            new_examples[key].append(example[key])
            cur_length = example['video'].shape[1]
    ...
    new_examples["render_mask_video"] = torch.stack(
        [example[:,:batch_video_length,:,:] for example in new_examples["render_mask_video"]])
    if "first_frame" in new_examples:
        new_examples["first_frame"] = torch.stack(
            [torch.tensor(example) for example in new_examples["first_frame"]])
    if "camera_extrinsics" in new_examples:
        new_examples["camera_extrinsics"] = torch.stack(
            [example[:batch_video_length,:,:] for example in new_examples["camera_extrinsics"]]
        )
    if "camera_intrinsics" in new_examples:
...
def train(args):
    ...
    dataset = CameraVideoDataset(
        ...
        None,
        # ['/mnt/data/hdd/user_workspace/duanke/video_mixkit_81f_26347/mixkit_vggt_results.json', '/mnt/data/hdd/user_workspace/yuqifan/cameractrl/vggt/openhumanvid_81frames_filtered.json'],
        steps_per_epoch=args.steps_per_epoch,
        max_num_frames=129,
        frame_interval=1,
        num_frames=81,
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[pl.pytorch.callbacks.ModelCheckpoint(save_top_k=-1, every_n_train_steps=1000, save_weights_only=False)],
        logger=logger,
