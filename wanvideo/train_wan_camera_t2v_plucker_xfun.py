...
class CameraXfunVideoDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, meta_path, camera_path, steps_per_epoch, max_num_frames=100, frame_interval=1, num_frames=81, height=480, width=832, is_i2v=False, is_camera=False):
        metadata = json.load(open(meta_path))
        camera_data = np.load(camera_path, allow_pickle=True)["arr_0"]  # 加载npz，拿arr_0
        camera_dict = dict()
        for entry in camera_data:
            camera_dict['_'.join(entry["video_path"].split('/')[-4:])] = entry
        print("total number of videos: ", len(metadata))

        self.text = [entry["text"] for entry in metadata]
        
        self.path = [entry["file_path"].replace('2002-hdd','camera_datasets') for entry in metadata]
        ...
        for entry in metadata:
            key = '_'.join(entry["file_path"].split('/')[-4:])
            self.camera_extrinsics.append(camera_dict[key]["camera_extrinsics"])
            self.camera_intrinsics.append(np.repeat(camera_dict[key]["camera_intrinsics"][np.newaxis, :], len(camera_dict[key]["camera_extrinsics"]), axis=0))
    ...
    def __getitem__(self, index):
        ...
        try: 
            if self.is_camera:
                # video_id = video_id[::4]
                data["camera_extrinsics"], data["camera_intrinsics"] = get_camera_sparse_embedding(self.camera_extrinsics[data_id][video_id], self.camera_intrinsics[data_id][video_id], self.height, self.width)
...
class TensorDataset(torch.utils.data.Dataset):
    def __init__(self, base_path, npz_path, steps_per_epoch):
        metadata = np.load(npz_path, allow_pickle=True)["arr_0"][:1000]  # 加载npz，拿arr_0
...
def train(args):
    ...
    #     args.dataset_path,
    #     os.path.join(args.dataset_path, 'MultiCamVideo_metricscale.npz'),
    #     steps_per_epoch=args.steps_per_epoch,
    # )
    dataset = CameraXfunVideoDataset(
        args.dataset_path,
        '/mnt/data/hdd/user_workspace/duanke/VideoX-Fun/datasets/mydataset/Muticam_filtered_dataset/filtered_data.json',
        '/mnt/data/camera_datasets/KwaiVGI/MultiCamVideo-Dataset/MultiCamVideo_metricscale.npz',
        steps_per_epoch=args.steps_per_epoch,
        max_num_frames=129,
        frame_interval=1,
        num_frames=81,
