...
i = 0
def calc_cammc(rt1, rt2):  # N, 3, 4
    return  (rt2 - rt1).reshape(-1, 12).norm(p=2, dim=-1)
image_folder = f"data/train_eval/temp_images"
device = "cuda" if torch.cuda.is_available() else "cpu"
vggt_model = VGGT.from_pretrained("/mnt/data/hdd/user_workspace/yuqifan/cameractrl/VGGT-1B").to(device)
update_data = []
while i < 5:
    data = total_data[i]
    print(data)
    data['path'] = data['path'].replace('yqf_camera_datasets', 'datasets/camera_datasets')
    path = data['path']
    camera = data['camera'][0]
    update_data.append(data)
    print(camera.shape)
    print(type(camera))
    new_path = f"data/train_eval/{i}_" + path.split('/')[-1]
    os.system(f"cp {path} {new_path}")

    extrinsic, intrinsic = generate_camera_params(vggt_model, new_path, image_folder, device)
    print(extrinsic.shape)
    extrinsic_trans, intrinsic_trans = get_camera_sparse_embedding(extrinsic, intrinsic, height=480, width=832)
    print(extrinsic_trans.shape)
    x = calc_cammc(extrinsic_trans[:,:3,:], camera[:,:3,:])
    print(x)
torch.save(update_data, 'data/val_5_data.pt')
...
def eval_single():
    ...
    gt_image_folder = './examples/cami2v_gt1/images'
    gt_save_path = './examples/cami2v_gt1'

    gt_extrinsic, _ = predict_cameras(gt_image_folder, gt_save_path, device, dtype)

    test_image_folder = './examples/cami2v_test1/images'
    test_save_path = './examples/cami2v_test1'
    ...
    trans_err, rot_err, cam_mc = calculate_pose_errors(gt_extrinsic, test_extrinsic)
